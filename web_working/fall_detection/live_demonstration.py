...
class LiveDemonstrator:
    def __init__(self, model_path, sequence_length=10, fall_threshold=0.8, consecutive_frames=3, cooldown=10):
        """Initialize the live demonstrator with a trained model and configurable fall detection parameters.
        
        Args:
            model_path: Path to the trained model
            sequence_length: Number of frames to consider for each prediction
            fall_threshold: Probability threshold for fall detection (default: 0.8)
            consecutive_frames: Number of consecutive frames needed for fall detection (default: 3)
            cooldown: Cooldown period in seconds after a fall alert (default: 10)
        """
        ...
        self.consecutive_frames_needed = consecutive_frames
        self.cooldown_period = cooldown
        self.high_prob_frames = deque(maxlen=consecutive_frames)  # Track consecutive high probability frames
        self.last_fall_time = 0  # Track time of last fall detection
    ...
    def make_prediction(self):
        """Make a fall prediction using the current frame buffer and conservative detection approach."""
        try:
            ...
            self.high_prob_frames.append(self.fall_probability >= self.fall_threshold)
            
            # Check for fall conditions:
            # 1. All recent frames exceed threshold
            # 2. Not in cooldown period
            current_time = time.time()
            if (len(self.high_prob_frames) == self.consecutive_frames_needed and
                all(self.high_prob_frames) and
                current_time - self.last_fall_time >= self.cooldown_period):
    ...
    def save_sequences(self):
        ...
        filename = f"predictions/demonstration_sequences_{timestamp}.json"
        ...
        data = {
            ...
            "metadata": {
                ...
                "model_path": os.path.basename(self.detector.model_path),
                "fall_detection_params": {
                    "fall_threshold": self.fall_threshold,
                    "consecutive_frames": self.consecutive_frames_needed,
        ...
        print(f"Fall detection parameters used:")
        print(f"  - Threshold: {self.fall_threshold}")
        print(f"  - Consecutive frames: {self.consecutive_frames_needed}")
    ...
    def run(self):
        """Main loop for the live demonstrator."""
        try:
            # Initialize display
            self.init_display()
    ...
    def update_display(self):
        ...
        try:
            ...
            # Draw title with fall probability, parameters, and recording status
            title = f" Fall Detection Monitor - Prob: {self.fall_probability:.1%} "
            title += f"[Thresh: {self.fall_threshold:.1f}, Frames: {self.consecutive_frames_needed}, Cool: {self.cooldown_period}s] "
            if self.recording:
                title += "[ Recording... ]"
...
def main():
    try:
        ...
        # Create and run demonstrator with configurable parameters
        demonstrator = LiveDemonstrator(
            model_path,
            fall_threshold=0.8,  # Default threshold
            consecutive_frames=3, # Require 3 consecutive frames
